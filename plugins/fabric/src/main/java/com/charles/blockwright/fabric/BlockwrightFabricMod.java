...
public final class BlockwrightFabricMod implements ModInitializer {
    ...
    private static void sendStartupHint(ServerPlayerEntity player) {
        ...
        for (String message : ControllerProcessManager.startupHintMessages(config, language)) {
    ...
    private static int webAddress(ServerCommandSource source) {
        ControllerProcessManager.ensureStartedAsync(config, gameDir);
        BlockwrightLanguage language = BlockwrightLanguage.fromSource(source);
        for (String message : ControllerProcessManager.webAddressMessages(config.controllerUrl, language)) {
            String line = message;
    ...
    private static int runChat(ServerCommandSource source, String text) {
        ...
        if (player == null) {
            source.sendError(Text.literal(language.text(
        ...
        CompletableFuture
                ...
                .exceptionally(error -> {
                    server.execute(() -> {
                        ServerPlayerEntity currentPlayer = server.getPlayerManager().getPlayer(playerName);
                        if (currentPlayer != null) {
                            BlockwrightLanguage currentLanguage = BlockwrightLanguage.fromPlayer(currentPlayer);
                            currentPlayer.sendMessage(
                                    Text.literal(currentLanguage.text(
    ...
    private static void executeDirectActions(
        ...
        if (JobPoller.hasPlaceBlocks(response.actions)) {
            ...
            player.sendMessage(Text.literal(language.text(
                    ...
                    "Blockwright 正在执行另一个建筑任务，请等它完成后再试。")), false);
    ...
    private static void sendScannedRetry(
        ...
        if (attempt >= MAX_SCAN_REPLAN_ATTEMPTS) {
            player.sendMessage(Text.literal(language.text(
                    ...
                    "Blockwright 连续扫描后仍未生成可执行方案，已停止，避免一直重复扫描。")), false);
            return;
        }
        ...
        if (text == null || text.isBlank()) {
            player.sendMessage(Text.literal(language.text(
                    "Blockwright finished scanning, but the original request is missing.",
                    "Blockwright 扫描完成，但缺少要继续处理的原始需求。")), false);
            return;
        }
                ...
                .exceptionally(error -> {
                    server.execute(() -> {
                        ...
                        if (currentPlayer != null) {
                            ...
                            currentPlayer.sendMessage(
                                    Text.literal(currentLanguage.text(
                                                    "Blockwright failed to continue after scanning: ",
                                                    "Blockwright 扫描后继续处理失败：")
                                            + rootMessage(error)),
                                    false);
                        }
                    });
