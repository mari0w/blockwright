use crate::{
    domain::types::{BlockOrigin, GameAction, PlayerPosition},
...
impl Planner {
    pub async fn plan(&self, input: PlannerInput, blueprints: &BlueprintStore) -> PlanResult {
        ...
        if text.contains("图片") || lower_text.contains("image") {
            return PlanResult {
...
mod tests {
    use super::*;
    use crate::domain::types::{Blueprint, BlueprintBlock, BlueprintSize, MaterialCount};

    #[tokio::test]
    async fn plans_diamond_sword() {
        let temp_dir = std::env::temp_dir().join("blockwright-planner-test-sword");
        let store = BlueprintStore::new(temp_dir).await.unwrap();
        let result = Planner
            .plan(
                PlannerInput {
        ...
        assert!(matches!(result.actions[0], GameAction::GiveItem { .. }));
    }

    #[tokio::test]
    async fn plans_house_from_blueprint_tag() {
        let temp_dir = std::env::temp_dir().join("blockwright-planner-test-house");
        let store = BlueprintStore::new(temp_dir).await.unwrap();
        store
            .save(Blueprint {
                id: "test-house".to_string(),
                name: "测试木屋".to_string(),
