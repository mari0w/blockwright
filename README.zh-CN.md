...
Blockwright 是面向 Minecraft Java 版的生产级 AI 控制层。装好 Fabric 模组后，玩家和运营可以用自然语言描述意图，让助手通过受控工具读取玩家和世界状态、执行支持的 Minecraft 命令操作、放置或改造方块、管理构建记录，并校验结果。

你可以通过 Web 页面打字、按住麦克风说话、在游戏内输入 `/bw`，也可以把 Element/Matrix、钉钉等聊天工具接进来。

## 主要能做什么

- 读取上下文：查看当前玩家、背包、手持物和附近世界方块。
- 执行受控游戏动作：发物品、调时间或天气、加效果、切模式，或者执行支持的命令。
- 带记录地建造和改造：先保存蓝图和构建记录，再放置或修改方块。
- 校验结果：执行端会回读世界方块，报告放置结果是否和记录一致。
...
| 命令 | 用途 |
| --- | --- |
| `/bw <需求>` | 发送一条自然语言指令，可以读取上下文、执行受控动作、建造、改造，或改变世界/玩家状态。 |
| `/bw web` | 在游戏聊天里显示 Web 端访问地址。 |

## 适合谁用

- 想用 AI 助手操作 Minecraft，而不是记住每条命令的玩家。
- 想把 Minecraft 世界接到 Web、语音或聊天工具里的服主/运营。
- 需要 AI 操作 Minecraft、并且希望执行过程可记录可校验的团队和玩家。
