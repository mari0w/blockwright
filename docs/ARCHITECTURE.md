...
- 对 Codex 暴露 Blockwright MCP 工具：读取玩家状态、扫描附近方块、给物品、放方块、执行安全命令、查询/保存/删除蓝图、查询/删除/搜索构建记录、入队受控动作。
...
执行端根据动作类型操作服务端世界 API。`give_item` 必须优先把物品切到目标玩家主手可见的快捷栏槽位；背包满时可以把旧手持物或多余物品掉落在玩家脚边，不能因为空间不足就让玩家改说法。`run_command` 必须经过命令白名单，不能执行高风险命令。
